...
def parse_proto(example_proto):
    ...
    raw_arr_kapre = raw_arr_norm[None,:,:]
    pdb.set_trace()
    raw_arr_stft = Spectrogram(n_dft=8, n_hop=1, padding='same', power_spectrogram=1.0,
                               ...
                               image_data_format='channels_last', name='stft_output')(raw_arr_kapre)  # (B x F x T x 1)
...
#    model.compile(optimizer=keras.optimizers.RMSprop(),  # Optimizer
    ...
    if plot is True:
        which_num = plot_idx
        # test_input = comp_train_kapre[which_num].reshape((1,71,1))
        # (predicted, _) = model.predict(test_input)
        # print("Predicted shape = ", predicted.shape)
        # predicted = predicted.reshape((71,1))
        # actual = raw_train[which_num].reshape((71,1))
        # plt.figure()
        # plt.title('Compressed...')
        # plt.plot(comp_train[which_num].reshape((71,1)))
        # if save is True:
