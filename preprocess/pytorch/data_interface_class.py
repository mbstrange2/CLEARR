...
class io_object:
    ...
    def __init__(self, time_window, chunk_size, inp_file='[]', output_data_batch='', 
                     save_path='[]', mat_filename='data000000.csv' ,bin_filename='data000000.bin', num_bits = 12, fs = 20000, overwrite = False):
        """
    ...
    def write_bin_file(self):
        ...
        mat_file_path = os.path.join(self.save_path + '/' + self.bin_filename)
    ...
    def write_mat_file(self):
        ...
        # Make sure binfile is being appended for batches and not overwritten
        mat_file_pointer = open(mat_file_path, "a+")
        
        # Write header for initial batch
        """if self.write_header == True:
            print(type(self.header))
            mat_file_pointer.write(self.header)"""
        
        # Write data (Note: since data is in amtrix forma, it needs to be traversed in a specific manner for the bin file to be recognized correctly as described in 'readme_binfile_writing.txt'. That's why tobytes('F') is needed)
        [x_length, y_length] = self.sample_combined.T.shape
        for x in range(0, x_length):
            for y in range(0, y_length):
                mat_file_pointer.write(str(self.sample_combined.T[x, y]) + ',')
            mat_file_pointer.write('\n') 
            
        mat_file_pointer.close()