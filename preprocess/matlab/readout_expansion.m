...
outputPath = '/Volumes/Scratch/Users/dantemur/data/2015-11-09-3/expansion_8/data000';
...
%parpool(12);     % sets maximum number of cores for parfor
rng(0,'twister');% sets the seed for predictability

% Save first 10 seconds of unmodified data
% define the newData array
newData = zeros(bufferSize,nElec+1,'int16');
newDataAvg = zeros(bufferSize,nElec+1,'int16');
rawData = rawFile.getData(startSample, min(bufferSize,nSamplesToCopy));
[offsetPerChannel] = offsetPerChannel_mex(rawData(:,2:end));
parfor i=1:min(bufferSize,nSamplesToCopy)
...
while nSamplesToCopy > 0
    ...
    parfor i=1:nSamples
        [newData(i,:)] = ramp_adc_expansion_mex(rawData(i,:),offsetPerChannel...
